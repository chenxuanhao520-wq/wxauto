...
class GLMEmbeddingUpgrader:
    ...
    def __init__(self):
        ...
        self.embedding_service = GLMEmbeddingService()
        
        logger.info("✅ GLM 嵌入服务升级器初始化成功")
    
    async def test_embedding_service(self):
        """测试嵌入服务"""
        try:
            logger.info("🔍 测试 GLM 嵌入服务...")
            ...
            for text in test_texts:
                ...
                if embedding:
                    logger.info(f"✅ GLM 测试成功: {len(embedding)} 维")
                else:
...
async def main():
    """主函数"""
    logger.info("🚀 GLM 嵌入服务升级 - 使用智谱 GLM 真实嵌入模型...")
    logger.info("=" * 60)
    
    try:
        # 初始化升级器
        upgrader = GLMEmbeddingUpgrader()
        ...
        if all_ok:
            logger.info("\n🎉 GLM 嵌入服务升级全部完成！")
            logger.info("💡 现在使用智谱 GLM 的真实AI嵌入模型")
            logger.info("💡 系统具备真正的语义搜索能力")
            logger.info("💡 向量维度: 1024 (GLM embedding-2)")
        else:
            logger.info("\n⚠️ 部分升级未完成")
